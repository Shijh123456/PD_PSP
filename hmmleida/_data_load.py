...
class DataLoad:
    ...
    def __init__(self,data_path='data',results_path='LEiDA_results'):
        ...
        #load concatenated and kmeans predictions dataframes.
        try:
            self.concatenated_data = pd.read_csv(f'{results_path}/concatenated_data.csv',sep='\t')
#            nrois = self.concatenated.shape[1]-2
        except:
...
#        self._N_classes_ = len(self._classes_lst_)
    ...
    def load_centroids(self,k=2):
        ...
        _check_k_input(self._K_min_,self._K_max_,k)
        try:
            centroids = pd.DataFrame(self.load_model(k=k).means_,columns=self.rois_labels)
        except:
    ...
    def group_static_fc(self,group=None,plot=True,cmap='jet',darkstyle=False, save_path=None):
        """
        ...
        for idx,sub in enumerate(subjects_ids):
            print(signals[sub][1:,1:].shape)
            pooled_static_fc[:,:,idx] = np.corrcoef(signals[sub][1:,1:])
        
        static_fc = np.nanmean(pooled_static_fc,axis=-1)
        static_fc = np.arctanh(static_fc)
        ...
        if plot:
            ...
            with plt.style.context('dark_background' if darkstyle else 'default'):
                ...
                if save_path is not None:
                    plt.savefig(f'{save_path}/5.group_static_fc_{group}.png')
                print(f"Figure saved to: {save_path}")
                plt.close()

        if save_path:
            try:
                static_fc = pd.DataFrame(static_fc, columns=self.rois_labels)
                static_fc.to_csv(f'{save_path}/static_fc_{group}.csv',index=False)
            except:
                print("Warning: An error ocurred when saving the 'static_fc.csv' file to local folder.")

        return static_fc

    def group_dynamics_fc(self, k=2, group=None, plot=True, cmap='jet', darkstyle=False,save_path=None,atlas='atlas'):
        if not isinstance(group, str):
            raise TypeError("'group' must be a string.")

        classes = load_classes(self._data_path_)
        y = self.predictions[f'k_{k}'].values
        ...
        for i in range(k):
            signals = self.concatenated_data
            x = pd.DataFrame(signals)
            x.insert(0, 'label', y)

            signal = load_tseries(self._data_path_)
            subjects_ids = [sub for sub, condition in zip(signal.keys(), conditions) if condition == group]
            N_subjects = len(subjects_ids)
            N_rois = len(self.rois_labels)
            pooled_static_fc = np.empty((N_rois, N_rois, N_subjects))
            ...
            for idx, sub in enumerate(subjects_ids):
                df = pd.DataFrame(x)
#                condition = (df['subject_id'] == sub and df['label'] == i+1)
                condition1 = (df['subject_id'] == sub)
                df = df.loc[condition1]
                condition2 = (df['label'] == i)
                ...
                sig = sig.T
                print(sig[:,1:].shape)
                pooled_static_fc[:, :, idx] = np.corrcoef(sig[:,1:])

            static_fc = np.nanmean(pooled_static_fc, axis=-1)
            if save_path:
                try:
                    mean_connectivity_matrix = pd.DataFrame(static_fc, columns=self.rois_labels)
                    mean_connectivity_matrix.to_csv(f'{save_path}/{atlas}_mean_connectivity_matrix_{group}_state{i+1}.csv', index=False)
                except:
                    print("Warning: An error ocurred when saving the 'mean_connectivity.csv' file to local folder.")
            static_fc = np.arctanh(static_fc)
            np.fill_diagonal(static_fc, 0)
            ...
            if plot:
                ...
                with plt.style.context('dark_background' if darkstyle else 'default'):
                    ...
                    # plt.show()
                    if save_path is not None:
                        plt.savefig(f'{save_path}/6.group_dynamics_fc_{group}_{i+1}.png')
                    print(f"Figure saved to: {save_path}")
                    plt.close()
            ...
            if save_path:
                try:
                    static_fc = pd.DataFrame(static_fc, columns=self.rois_labels)
                    static_fc.to_csv(f'{save_path}/group_dynamics_fc_{group}_{i+1}.csv', index=False)
                except:
                    print("Warning: An error ocurred when saving the 'static_fc.csv' file to local folder.")



    def group_transitions(self,k=2,metric='mean',cmap='YlGnBu',darkstyle=False,save_path=None):
        """
...
#        data = pd.DataFrame(self.load_model(k=k).transmat_)
    ...
    def overlap_withyeo(self,parcellation=None,n_areas=None,k=2,state=None,darkstyle=False,save_path=None,set_negative_zero=True):
        """
        ...
        corr,pvals = rsnets.compute_overlap(
            ...
            n_areas=n_areas,
            set_negative_zero=set_negative_zero
            )
    ...
    def plot_states_nodes(self,k=2,state='all',node_size=15,show_labels=True,open=True,save=False):
        ...
        the PL pattern are coloured in re
        d, and the rest
    ...
    def barplot_centroids(self,k=2,state='all',save_path=None):
        """
        ...
        if state=='all':
            barplot_states(centroids,self.rois_labels,save_path=save_path)
        else:
            _check_state(k,state)
    ...
    def plot_states_network_glass(self,k=2,darkstyle=False,save_path=None):
        """
    ...
    def plot_states_in_bold(self,subject_id,k=2,alpha=.5,darkstyle=False,save_path=None):
        """
        ...
        _check_k_input(self._K_min_, self._K_max_, k)
        signals = load_tseries(self._data_path_)
        signals = signals[subject_id][1:,:]  # get subject signals (and exclude 1st and last volumes)
        y = self.predictions[self.predictions.subject_id == subject_id][
            f'k_{k}'].values  # get predictions for selected k.

        with plt.style.context('dark_background' if darkstyle else 'default'):
            states_in_bold(signals, y, alpha=alpha,save_path=save_path)
    ...
    def plot_state_dwell_occupy(self,k=2,darkstyle=False,save_path=None,psp_jia=None,abnormal_state=1):
        ...
        for state in range(1,k+1):
            ...
            occ['state'] = state
            dt['state'] = state
            df_occ = pd.concat([df_occ, occ], ignore_index=True)
            df_dt = pd.concat([df_dt, dt], ignore_index=True)
    ...
    def explore_state(self,k=2,state=1,darkstyle=False,save_path=None):
        """
    ...
    def individual_dynamics_fc(self, k=2,atlas='atlas',state=1):
        classes = load_classes(self._data_path_)
        y = self.predictions[f'k_{k}'].values
        ...
        signal = load_tseries(self._data_path_)
        subjects_ids = [sub for sub in signal.keys()]
        N_subjects = len(subjects_ids)
        N_rois = len(self.rois_labels)
        pooled_static_fc = np.empty((N_rois, N_rois, N_subjects))
        erro_list = []
        ...
        for idx, sub in enumerate(subjects_ids):
            ...
            print(sig[:, 1:].shape)
            corr = np.corrcoef(sig[:, 1:])
            if np.any(np.all(corr == 0, axis=1)):
                raise ValueError("pooled_static_fc contains a row of all zeros.")
                # 检查是否有 NaN 值
            if np.any(np.isnan(corr)):
                print(sub,state,"pooled_static_fc contains NaN values.")
                erro_list.append(idx)
                sub_erro_list.append(sub)
                continue
            pooled_static_fc[:, :, idx] = corr

        print(pooled_static_fc.shape)
        kongzhi = 0
        for n in range(pooled_static_fc.shape[2]):
            if n in erro_list:
                print(n)
                pooled_static_fc = np.delete(pooled_static_fc,n-kongzhi,axis=2)
                kongzhi = kongzhi+1
        print(pooled_static_fc.shape)
        if np.any(np.isnan(pooled_static_fc)):
            raise ValueError("pooled_static_fc contains NaN values.")
        if np.any(np.all(pooled_static_fc == 0, axis=1)):
            raise ValueError("pooled_fc contains a row of all zeros.")
        fc = [None] * pooled_static_fc.shape[2]
        for m in range(pooled_static_fc.shape[2]):
            fc[m] = pooled_static_fc[:,:,m]

        return fc, sub_erro_list

    def individual_static_fc(self, atlas='atlas'):
        classes = load_classes(self._data_path_)
        ...
        erro_list, sub_erro_list = [], []

        for idx, sub in enumerate(subjects_ids):
            signal[sub][1:, 1:] = np.array(signal[sub][1:, 1:], dtype=np.float64)
            signal[sub][1:, 1:] = np.nan_to_num(signal[sub][1:, 1:], nan=0.0)
            if np.any(np.all(signal[sub][1:, 1:] == 0, axis=1)):
                print(sub,"pooled_static_fc contains NaN values.")
                erro_list.append(idx)
                sub_erro_list.append(sub)
                continue
            pooled_static_fc[:, :, idx] = np.corrcoef(signal[sub][1:, 1:])
