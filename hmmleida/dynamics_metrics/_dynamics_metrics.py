...
def group_transition_matrix(transitions_df,metric='mean',plot=True,cmap='inferno',darkstyle=False,save_path=None):
    """
    ...
    N_clusters = np.sqrt(len(transitions_df.columns[2:])).astype(int)
    #N_clusters = len(transitions_df.columns[:]) #get the number of clusters/states.
    mats = {}
#    current_group_matrix = transitions_df
...
#    if conds == 1:
        np.fill_diagonal(current_group_matrix, np.nan)
        if plot:
            #plotting
            with plt.style.context('dark_background' if darkstyle else 'default'):
                ...
                sns.heatmap(
                    ...
                    #annot=True,
                    square=True,
                    cbar_kws={"shrink": 1.4,"label":"Transition\nprobability"},
                    vmin=0,
                    vmax=0.14,
                    center=0.07,
                    fmt='.2f'
...
#                plt.title('HC', fontweight='regular', fontsize=18)
                plt.tight_layout()
#                plt.show()
                if save_path is not None:
                    plt.savefig(f'{save_path}/3.group_transitions_{cond}_index.png')
